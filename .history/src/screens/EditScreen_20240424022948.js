...
export default function EditScreen() {
    ...
    const updateImage = async () => {
        try {
            const imagesRef = collection(db, 'Images'); // Reference to the 'Images' collection
            const q = query(imagesRef, where("imageUri", "==", imageUri)); // Query documents where 'imageUri' matches the provided image URI
            const querySnapshot = await getDocs(q);
            querySnapshot.forEach(async (doc) => {
                // For each matching document, update the title field
