...
import { collection, query, getDocs, doc, setDoc, getDoc, serverTimestamp, where, collectionGroup} from 'firebase/firestore';
...
export default function EditScreen() {
    ...
    console.log("Edit UserId " + imageUri);
    useEffect(() => {
        setImage(imageUri);
        seth1Title(text);
    ...
    const updateImage = async () => {
        try {
            const q = query(collectionGroup(db, 'Images'), where("imageUri", "==", imageUri)); // Collection group query to search across all 'Images' subcollections
            ...
            if (!querySnapshot.empty) {
                ...
                await updateDoc(doc.ref, { title: title });
